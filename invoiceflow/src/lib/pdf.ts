...
function renderClassic(invoice: InvoiceData): string {
  ...
  <div class="footer"><span>Generated by Invoicely</span><span>${escapeHtml(invoice.business.email)}</span></div>
  </body></html>`;
}
...
function renderModern(invoice: InvoiceData): string {
  ...
  <div class="footer"><span>Invoicely</span><span>${escapeHtml(invoice.business.email)}</span></div>
  </body></html>`;
}
